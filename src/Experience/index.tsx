...
export default function Experience({
  ...
  return (
    <div
      className="mb-3 cursor-pointer select-none rounded-lg group bg-[#1a1a1a] border border-[#2a2a2a] p-4 hover:border-[#3a3a3a] transition-colors"
    ...
    >
        ...
        <div className="flex items-start justify-between w-full min-w-0">
          <div className="flex flex-col text-left">
            ...
            <h4 className="text-white/70 text-sm">
              <a href={link} className="!text-white/70 hover:!text-white/90 transition-colors">
                {company}
              </a>
            </h4>
            <span className="text-white/50 text-sm mt-0.5">{location}</span>
          </div>
          <div className="flex items-center gap-2 shrink-0 ml-4">
            <span
              className="text-sm text-white/50 hidden sm:block"
            ...
            <svg
              className={`transition-transform duration-300 w-4 h-4 text-white/50 shrink-0 ${
                isOpen ? "rotate-180" : ""
              }`}
              viewBox="0 0 20 20"
              fill="none"
      ...
      >
        <div className="pt-4 pl-16">{children}</div>
      </div>
    </div>
  );
}
